...
use stm32l4xx_hal::serial::Rx;
use stm32l4xx_hal::serial::Tx;
use stm32l4xx_hal::stm32::Peripherals;

//const _BAUD: u32 = 460_800;
const _BAUD: u32 = 230_400;
...
fn main() -> ! {
    ...
    // Initialize and test the UART
    let (_tx, _rx) = test_uart(stim, _BAUD);
    ...
    //let _cp;
    //unsafe {
    //    _cp = cortex_m::Peripherals::steal();
    //}
    //let mut _timer = Delay::new(_cp.SYST, 16_000_000);
    led.set_high();
...
fn test_uart(
    ...
    Tx<stm32l4xx_hal::pac::USART2>,
    Rx<stm32l4xx_hal::pac::USART2>,
) {
    let (mut tx, _rx) = uart_stm32::uart_init(_stim, _BAUD);

    iprintln!(_stim, "uart_tx_test, logging via ITM\n");

    let buffer = b"\n\rHello World, over UART\n\r";
    let _result = tx.bwrite_all(buffer);
    iprintln!(
        _stim,
        "Check the uart, running at {} baud for the blow string:\n",
        _BAUD
    );
    iprintln!(_stim, "{:?}\n", buffer);
...
{
    let dp: stm32l4xx_hal::stm32::Peripherals;
    unsafe {
